...
namespace Syndra
{
    internal class Program
    {
        ...
        private static void Game_OnGameLoad(EventArgs args)
        {
            ...
            EQ.SetSkillshot(float.MaxValue, 55f, 2100f, false, SkillshotType.SkillshotLine);
        ...
        private static void UseSpells(bool useQ, bool useW, bool useE, bool useR, bool useQE, bool useIgnite, bool isHarass)
        {
            ...
            var RDamage = rTarget != null ? GetRDamage(rTarget) : 0;
            var igniteDamage = rTarget != null ? GetigniteDamage(rTarget) : 0;
            ...
            if (rTarget != null && useR && RDamage > rTarget.Health)
            {
                if (R.IsReady())
                {
                    R.Cast(rTarget);
            ...
            if (rTarget != null && useR && (Config.Item("HarassActive").GetValue<KeyBind>().Active || Config.Item("LaneClearActive").GetValue<KeyBind>().Active) && Config.Item("ComboActive").GetValue<KeyBind>().Active)
            {
                if (R.IsReady())
                {
                    Q.CastIfHitchanceEquals(qTarget, HitChance.High);
